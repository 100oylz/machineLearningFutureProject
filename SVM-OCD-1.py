...
class SVM:
    ...
    def __init__(self, C=1.0, kernel='gaussian', tol=1e-3, max_iter=500):
        self.C = C
        self._kernel = kernel
        self.tol = tol
    ...
    def fit(self, X, y):
        ...
        self.b = 0.0
        self.errors = self.decision_function(self.X) - y
        self._optimize()
...
#----------------------------------降维处理数据集------------------------------------------
    data, label, labelMap = utils.generateDataSet(OCD_fMRI)
    data = np.array(data)
