...
class SVM:
    ...
    def __init__(self, C=1.0, kernel='poly', tol=1e-3, max_iter=100):
        self.C = 1.0
        self._kernel = kernel
        self.tol = tol
...
#----------------------------------降维处理数据集------------------------------------------
    data, label, labelMap = utils.generateDataSet(PPMI)
    ...
    print(y)
#----------------------------------------------------过采样处理--------------------------------------------------
    # PPMI_fMRI = utils.reshapeDataDict(PPMI_fMRI)
    # x_train, y_train, valid_data, valid_label, x_test, y_test, labelMap=utils.train_valid_test_split(PPMI_fMRI,42)

    for i in range(10):
       pca = PCA(n_components=0.927)
...
#-----------------------------------------训练数据集-------------------------------------------------------
       ...
       print(i,train(x_train,y_train,x_train,y_train))